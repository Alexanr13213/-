...
{
	cout << "INFO:" << endl;
	cout << "Кол-во задач: " << countTask << endl;
	if (countTask != 0)
		cout << "Кол-во отказов: " << (waitQue * 100 / countTask) << "%" << endl;
	else
		cout << "Кол-во отказов: " << 0 << endl;
	cout << "Потрачено впустую: " << (waitCPU * 100 / time) << "%" << endl;
	if (done != 0)
		cout << "Время работы: " << (double(time - waitCPU) / double(done)) << endl;
	else
		cout << "Время работы: " << 0 << endl;
	cout << "Всего выполнено задач: " << done << endl;
	cout << "Кол-во циклов: " << CycleCount << endl;
}