...
search_button.addEventListener('click', search_ButtonClick);
home_button.addEventListener('click', Home_ButtonClick);

function Home_ButtonClick(){
    fetch('/', {
        method: 'GET'
    }) .then(respons => {
...
async function search_ButtonClick(){
...
function updateTable(json){
    ...
    for(let i = 0; i < json.length; i++){
        ...
        tdTag = document.createElement('td');
        for(let j = 0; j < json[i].selected_marks.length; j++){
            if(json[i].selected_marks.length != 1){
                if(j != json[i].selected_marks.length - 1)
                    tdTag.innerHTML += json[i].selected_marks[j] + ', ';
                    else tdTag.innerHTML += json[i].selected_marks[j];  
            } else {
                tdTag.innerHTML = json[i].selected_marks[j];
            }
        }
